...
saveeverything() {
	...
	echo "PlatformClass:$PLATFORMCLASS" >> ./src/platformsupport/$PLATFORMNAME/vars.mcfg
	echo "PlatformHeader:$PLATFORMCLASSHEADER" >> ./src/platformsupport/$PLATFORMNAME/vars.mcfg
	...
	echo "ARM:$ARMDEVICESTATUS" >> ./src/platformsupport/$PLATFORMNAME/vars.mcfg
...
if test "$SELECTEDACTIONID" = "1" || test "$SELECTEDACTIONID" = "2"; then
	...
	if test "$SELECTEDACTIONID" = "2"; then
		...
		printf "Is this an ARM-based microcontroller? Type \"yes\" or \"no\". "
		read tmp_devicearmcortex
		if test "$tmp_devicearmcortex" = "yes" || test "$tmp_devicearmcortex" = "Y" || test "$tmp_devicearmcortex" = "y" || test "$tmp_devicearmcortex" = "YES"; then
			printf "Which CPU does it have? (e.g. 64/cortex-a53 or arm9) "
			read tmp_devicearmcpu
			if test "$tmp_devicearmcpu" = ""; then
				tmp_devicearmcpu="arm7"
			elif test "$tmp_devicearmcpu" = "64"; then
				tmp_devicearmcpu="cortex-a53"
			elif test "$tmp_devicearmcpu" = "32"; then
				tmp_devicearmcpu="arm7"
			fi
			ARMDEVICESTATUS="$tmp_devicearmcpu"
		else
			ARMDEVICESTATUS=no
		fi
...
while read linefrommcfg; do
	...
	if test "$HALFONE" = "OS"; then
		PLATFORMOS="$HALFTWO"
	elif test "$HALFONE" = "ARM"; then
		ARMDEVICESTATUS="$HALFTWO"
	elif test "$HALFONE" = "PlatformID"; then
		PLATFORMDEFINE=`echo "$HALFTWO" | sed 's/MULE_PLATFORM_//g'`
	elif test "$HALFONE" = "PlatformClass"; then
		PLATFORMCLASS="$HALFTWO"
	elif test "$HALFONE" = "PlatformHeader"; then
		PLATFORMCLASSHEADER="$HALFTWO"
